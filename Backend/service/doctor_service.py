...
async def update_appointment_status(ma_lich_hen: str, trang_thai: str):
    ...
    try:
        cursor = await conn.execute(
            "UPDATE LICH_HEN SET TrangThai = ? WHERE MaLichHen = ?",
...
async def save_examination_record(exam_data: dict):
    ...
    try:
        ...
        cursor = await conn.execute("SELECT MaLuotKham FROM LUOT_KHAM WHERE MaLichHen = ?", (ma_lich_hen,))
        ...
        if row:
            ...
            await conn.execute("""
                UPDATE LUOT_KHAM 
                SET TrieuChung = ?, LoiDan = ?, MaBenh = ?
                WHERE MaLuotKham = ?
            """, (exam_data.get('trieu_chung'), exam_data.get('loi_dan'), ma_benh, ma_luot_kham))

            await conn.execute("DELETE FROM CHI_TIET_XET_NGHIEM WHERE MaLuotKham = ?", (ma_luot_kham,))
            await conn.execute("DELETE FROM CHI_TIET_DON_THUOC WHERE MaLuotKham = ?", (ma_luot_kham,))
            await conn.execute("DELETE FROM LICH_TRINH_DIEU_TRI WHERE MaLuotKham = ?", (ma_luot_kham,))
        else:

            cursor_max = await conn.execute("""
                SELECT MaLuotKham FROM LUOT_KHAM 
                ORDER BY CAST(SUBSTR(MaLuotKham, 4) AS INTEGER) DESC LIMIT 1
            """)
            last_lk = await cursor_max.fetchone()
            
            if last_lk and "_" in last_lk[0]:
                try:
                    last_num = int(last_lk[0].split("_")[1])
        ...
        if xet_nghiem_hop_le:
            cur_xn = await conn.execute("""
                ...
                FROM CHI_TIET_XET_NGHIEM CTXN JOIN DICH_VU DV ON CTXN.MaDichVu = DV.MaDichVu WHERE CTXN.MaLuotKham = ?
            """, (ma_luot_kham,))
        ...
        if don_thuoc_hop_le:
            cur_thuoc = await conn.execute("""
                ...
                FROM CHI_TIET_DON_THUOC CTDT JOIN THUOC T ON CTDT.MaThuoc = T.MaThuoc WHERE CTDT.MaLuotKham = ?
            """, (ma_luot_kham,))
        ...
        if dieu_tri_hop_le:
            cur_dt = await conn.execute("""
                ...
                FROM LICH_TRINH_DIEU_TRI LTDT JOIN DICH_VU DV ON LTDT.MaDichVu = DV.MaDichVu WHERE LTDT.MaLuotKham = ?
            """, (ma_luot_kham,))
